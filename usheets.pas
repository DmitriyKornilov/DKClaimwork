...
type
  ...
  public
    ...
    procedure Update(const AMotorNames, AMotorNums, AMotorDates, AMileages,
...
begin
  ...
  AddToHeader(2, 4, 'Изготовлен');
  //рекламации
  AddToHeader(1, 5, 2, 5, 'Предприятие');
  AddToHeader(1, 6, 2, 6, 'Пробег локомотива, км');
  AddToHeader(1, 7, 2, 7, 'Отправитель уведомления о неисправности');
  AddToHeader(1, 8, 2, 8,    LETTER_NAMES[0]);
  AddToHeader(1, 9, 2, 9,    LETTER_NAMES[1]);
  AddToHeader(1, 10, 2, 10,  LETTER_NAMES[2]);
  AddToHeader(1, 11, 2, 11,  LETTER_NAMES[3]);
  AddToHeader(1, 12, 2, 12,  LETTER_NAMES[4]);
  AddToHeader(1, 13, 2, 13,  LETTER_NAMES[5]);
  AddToHeader(1, 14, 2, 14, 'Примечание по ходу расследования');
  AddToHeader(1, 15, 2, 15, 'Статус рекламации');
  //гарантийный ремонт
  AddToHeader(1, 16, 2, 16, 'Отправитель запроса на согласование ремонта');
  AddToHeader(1, 17, 2, 17,    LETTER_NAMES[6]);
  AddToHeader(1, 18, 2, 18,    LETTER_NAMES[7]);
  AddToHeader(1, 19, 2, 19,    LETTER_NAMES[8]);
  AddToHeader(1, 20, 2, 20,    LETTER_NAMES[9]);
  AddToHeader(1, 21, 2, 21, 'Дата прибытия в ремонт');
  AddToHeader(1, 22, 2, 22, 'Дата отправки из ремонта');
  AddToHeader(1, 23, 2, 23, 'Примечание по ходу ремонта');
  AddToHeader(1, 24, 2, 24, 'Статус ремонта');
  //возмещение затрат
  AddToHeader(1, 25, 2, 25, 'Отправитель претензии');
  AddToHeader(1, 26, 2, 26, LETTER_NAMES[10]);
  AddToHeader(1, 27, 2, 27, 'Сумма к возмещению');
  AddToHeader(1, 28, 2, 28, LETTER_NAMES[11]);
  AddToHeader(1, 29, 2, 29, LETTER_NAMES[12]);
  AddToHeader(1, 30, 2, 30, LETTER_NAMES[13]);
  AddToHeader(1, 31, 1, 32, 'Компенсировано Потребителю');
  AddToHeader(2, 31, 'Дата');
  AddToHeader(2, 32, 'Сумма');
  AddToHeader(1, 33, 1, 34, 'Возмещено Производителем');
  AddToHeader(2, 33, 'Дата');
  AddToHeader(2, 34, 'Сумма');
  AddToHeader(1, 35, 2, 35, 'Примечание по возмещению затрат');
  AddToHeader(1, 36, 2, 36, 'Статус претензионной работы');
end;

procedure TLogTable.Update(const AMotorNames, AMotorNums, AMotorDates, AMileages,
                           AReclamationLocations, AReclamationUsers,
                           AReclamationNoticeFromUserStrs,
                           AReclamationNoticeToBuilderStrs,
                           AReclamationAnswerFromBuilderStrs,
...
begin
  ...
  SetColumnString('Пробег', AMileages);
